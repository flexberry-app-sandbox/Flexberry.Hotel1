 ...
 */
@Entity(name = "IISHotelРегистрЗаписи")
...
public class RegistrZapisi {
    ...
    @EdmIgnore
    @Converter(converterClass = UUIDConverter.class, name = "ZapisGostya")
    @Convert("ZapisGostya")
    @Column(name = "ЗаписьГостя", length = 16, unique = true, nullable = false)
    private UUID _zapisgostyaid;

    @ManyToOne(optional = false, fetch = FetchType.LAZY)
    @JoinColumn(name = "ZapisGostya", insertable = false, updatable = false)
    private ZapisGostya zapisgostya;
